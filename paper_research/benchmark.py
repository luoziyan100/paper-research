...
from paper_research.web_search import extract_duckduckgo_results as _extract_duckduckgo_results
from paper_research.web_search import fetch_url as _fetch_url
...
def _infer_report_strengths(content: str, language: str = "en") -> List[str]:
    ...
    if "reproduc" in lower or "replication" in lower or "复现" in content:
        strengths.append(
            "检查可复现性和复现实验细节。"
            if language == "zh"
            else "Checks reproducibility and replication details."
        )
